    ...
    public async createAppointment(appointmentReq: AppointmentReq): Promise<Appointment> {
        try {
            ...
            if (doctor) {
                ...
                logger.error(`Doctor ${appointmentReq.id_doctor} doesn't exist`)
                throw new CreationError("Docotr dont found", "Appointment")
            }
        } catch (error) {
            throw new CreationError("Appointment creation failed", "Appointment")
        }
    }
