...
describe('DoctorController', () => {
    ...
    describe('deleDoctorById', () => {
        it('should delete doctor by id', async () => {
            //Mock Process
            const doctorRes: Doctor = { id_doctor: 23, nombre: 'Andres', apellido: 'Rocky', especialidad: 'Medicina general', consultorio: 2 };
            (mockReq.params) = { id: "23"};
            (doctorSerivce.deleteDoctor as jest.Mock).mockResolvedValue(doctorRes);

            // METHOD Execution 
            await doctorController.deleteDoctor(mockReq, mockRes)

            //AssertS
            expect(doctorSerivce.deleteDoctor).toHaveBeenCalledWith(23)
            expect(mockRes.json).toHaveBeenCalledWith({message:"Doctor was deleted successfully"})
            expect(mockRes.status).toHaveBeenCalledWith(200)
        })
    })
})